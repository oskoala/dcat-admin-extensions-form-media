...
$(function () {
    var LakeFormMedia = {
        init: function () {
            ...
            this.onEvent('click', ".lake-form-media-delete-button", function (res) {
                var mediaModalCont = $(this).parents('.lake-form-media-modal');
                var mediaId = mediaModalCont.data('media');
                var mediaCont = $("." + mediaId);
                var options = mediaCont.data('options');
                ...
                let selected_items = $(".lake-form-media-selected")
                if (selected_items.length === 0) {
                    if (currentPath === "/") {
                        toastr.error("请选择要删除的文件");
                        return;
                    } else {
                            ...
                            $.ajax({
                                ...
                                success: function (data) {
                                    if (data['code'] == 200) {
                                        ...
                                        let str = currentPath.substring(0, index);
                                        thiz.getdata(name, str, options)
                                    } else {
                                        toastr.error(data['msg']);
                                    }
                                },
